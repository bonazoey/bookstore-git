	...
	<div class="btn_log">
		<c:choose>
			<c:when test="${sessionScope.mvo == null }">
				<button type="button" id="login" name="login" class="btn btn-success" onclick="location.href='member?cmd=login'">로 그 인</button>
				<button type="button" id="register" name="register" class="btn btn-warning" onclick="">회원가입</button>
			</c:when>
			<c:otherwise>
				${sessionScope.mvo.name}님 환영합니다람쥐!
				<button type="button" id="logout" name="logout" class="btn btn-success" onclick="location.href='member?cmd=logout'">로그아웃</button>
			</c:otherwise>
		</c:choose>
	</div>